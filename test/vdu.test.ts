...
describe("VDU response helpers", () => {
  ...
  it("treats word_accent `message: false` as a genuine negative", async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValueOnce(
        new Response('<script>{"NONCE":"abcdef123456"}</script>'),
      )
      .mockResolvedValueOnce(Response.json({ code: 200, message: false }));

    vi.stubGlobal("fetch", fetchMock);

    await expect(lookupWordVariants("waver")).resolves.toEqual([]);
